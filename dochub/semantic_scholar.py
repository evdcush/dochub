...
# Data excluded from processed API response
DEFAULT_EXCLUSIONS = set(['citationVelocity', 'citations', 'influentialCitationCount',
                          'paperId', 'references', 'venue', 'url'])


def query_semantic_scholar(ref_id, unknown_ref=False):
    ...
    """
    #==== Format request URL
    scrub_id = utils.scrub_arx_id(ref_id)
    rid = f"arXiv:{scrub_id}" if utils.is_arxiv_id(scrub_id) else ref_id
    refs = "?include_unknown_references=" + str(unknown_ref).lower()
    req_url = SS_API_URL + rid + refs
    #print(ref_url)

    #==== Query API
    response = requests.get(req_url)
    if response.status_code != 200:
        print(f"  {response.status_code}: query unsuccessful for ref id {rid}")
        return
    return response.json()


def parse_semantic_scholar(response, exclude=DEFAULT_EXCLUSIONS):
    """ parses semantic scholar API response for relevant bibtex info

    Unlike the ArXiv API, the Semantic Scholar API has a very well organized
    and dense response for a single paper. Meaning, the response data can
    be used as-is and the only "processing" required is formatting the data
    to one's needs (bibtex entries, in this context).
    """
    info = OrderedDict()
...
class Query:
    ...
    def format_query(self, ref_id):
        ...
        if self.is_arxiv_id(scrubbed_id):
            ...
            self.urlPDF = f"{self.arx_url}/pdf/{scrubbed_id}"
        else:
    ...
    def parse(self):
        ...
        self.identifier = f"{self.authors[0].split(' ')[-1].lower()}{self.year}"
        info = OrderedDict()
